...
public class UsersDao {
	...
	public static int updateUser(HashMap<String, Object> map) {
		String sql = "update `Users` set Email = ?,FirstName = ?,LastName = ?,Gender = ?,DateOfBirth = ?,Phone = ?,Photo = ?,Address = ?,RoleId = ?"
				+ " where UserId = ?";
		return MySqlHelper.executeUpdate(sql, new Object[] {map.get("email"),map.get("firstName"),map.get("lastName"),
															map.get("gender"),map.get("dateOfBirth"),map.get("phone"),
															map.get("photo"),map.get("address"),map.get("roleId"),map.get("userId")});
	}
	...
	public static int addUser(HashMap<String, Object> map) {
		...
		return MySqlHelper.executeUpdate(sql, new Object[] {
				map.get("email"),map.get("password"),map.get("firstName"),
				map.get("lastName"),map.get("gender"),map.get("dateOfBirth"),
				map.get("phone"),map.get("photo"),map.get("address"),map.get("roleId")});
	}
	...
	public static List<HashMap<String, Object>> findUserListByPageAndRoleId(String name,int startPage,int pageSize,int roleId){
